...
#include <iostream>
namespace factory {
SFMLFactory::SFMLFactory(sf::RenderWindow &window, std::string texture_input,
...
std::shared_ptr<entities::Wall> SFMLFactory::createWall(float x, float y) {
  ...
  wall->setView(std::make_unique<views::SFMLWallView>(texture, atlas, wall,
                                                      window, camera));
  return wall;
}
std::shared_ptr<entities::Orb> SFMLFactory::createOrb(float x, float y) {
  auto orb = std::make_shared<entities::Orb>(x, y);
  orb->setView(std::make_unique<views::SFMLOrbView>(texture, atlas, orb, window,
                                                    camera));
  return orb;
}
std::shared_ptr<entities::PowerOrb> SFMLFactory::createPowerOrb(float x,
                                                                float y) {
  auto powerorb = std::make_shared<entities::PowerOrb>(x, y);
  powerorb->setView(std::make_unique<views::SFMLPowerOrbView>(
      texture, atlas, powerorb, window, camera));
  return powerorb;
}
std::shared_ptr<entities::Fruit> SFMLFactory::createFruit(float x, float y) {
  ...
  case 0: {
    ...
    fruit->setView(std::make_unique<views::SFMLCherryView>(
        texture, atlas, fruit, window, camera));
    return fruit;
  }
  case 1: {
    auto fruit = std::make_shared<entities::Strawberry>(x, y);
    fruit->setView(std::make_unique<views::SFMLStrawberryView>(
        texture, atlas, fruit, window, camera));
    return fruit;
  }
  case 2: {
    auto fruit = std::make_shared<entities::Orange>(x, y);
    fruit->setView(std::make_unique<views::SFMLOrangeView>(
        texture, atlas, fruit, window, camera));
    return fruit;
  }
  case 3: {
    auto fruit = std::make_shared<entities::Apple>(x, y);
    fruit->setView(std::make_unique<views::SFMLAppleView>(texture, atlas, fruit,
                                                          window, camera));
    return fruit;
  }
  case 4: {
    auto fruit = std::make_shared<entities::Melon>(x, y);
    fruit->setView(std::make_unique<views::SFMLMelonView>(texture, atlas, fruit,
                                                          window, camera));
    return fruit;
  }
  case 5: {
    auto fruit = std::make_shared<entities::Galaxian>(x, y);
    fruit->setView(std::make_unique<views::SFMLGalaxianView>(
        texture, atlas, fruit, window, camera));
    return fruit;
  }
  case 6: {
    auto fruit = std::make_shared<entities::Bell>(x, y);
    fruit->setView(std::make_unique<views::SFMLBellView>(texture, atlas, fruit,
                                                         window, camera));
    return fruit;
  }
  case 7: {
    auto fruit = std::make_shared<entities::Key>(x, y);
    fruit->setView(std::make_unique<views::SFMLKeyView>(texture, atlas, fruit,
                                                        window, camera));
    return fruit;
  }
  default:
...
std::shared_ptr<entities::Ghost> SFMLFactory::createGhost(
    float x, float y, std::shared_ptr<entities::Pacman> pacman,
    std::vector<std::vector<bool>> wallGrid, int id, bool first_time) {
  std::shared_ptr<entities::Ghost> ghost;
  switch (id) {
  case 0:
    ghost = std::make_shared<entities::LockedGhost>(x, y, pacman, wallGrid, id,
                                                    0.f);
    ghost->setView(std::make_unique<views::SFMLLockedGhostView>(
        texture, atlas, ghost, window, camera));
    break;
  case 1:
    ghost =
        std::make_shared<entities::ChaseGhost>(x, y, pacman, wallGrid, id, 0.f);
    ghost->setView(std::make_unique<views::SFMLChaseGhostView>(
        texture, atlas, ghost, window, camera));
    break;
  case 2:
    ghost = std::make_shared<entities::PredictGhost>(x, y, pacman, wallGrid, id,
                                                     first_time ? 5.f : 0.f);
    ghost->setView(std::make_unique<views::SFMLPredictGhost1View>(
        texture, atlas, ghost, window, camera));
    break;
  case 3:
    ghost = std::make_shared<entities::PredictGhost>(x, y, pacman, wallGrid, id,
                                                     first_time ? 10.f : 0.f);
    ghost->setView(std::make_unique<views::SFMLPredictGhost2View>(
        texture, atlas, ghost, window, camera));
    break;
  }
  return ghost;
}
std::shared_ptr<entities::Pacman> SFMLFactory::createPacman(float speed,
                                                            int mapwidth,
                                                            int mapheight,
                                                            float x, float y) {
  auto pacman =
      std::make_shared<entities::Pacman>(speed, mapwidth, mapheight, x, y);
  pacman->setView(std::make_unique<views::SFMLPacmanView>(
      texture, atlas, pacman, window, camera));
  return pacman;
}
