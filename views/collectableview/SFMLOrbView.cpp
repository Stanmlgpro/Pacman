...
namespace views {
    SFMLOrbView::SFMLOrbView(const sf::Texture& texture, std::shared_ptr<sprites::SpriteAtlas> atlas, std::weak_ptr<entities::Entity> entity, sf::RenderWindow& window, std::shared_ptr<Camera> camera) : SFMLView(texture, atlas, entity, window, camera) {
        FindSprite();
    ...
    void SFMLOrbView::FindSprite() {
        sprite.setTextureRect(atlas->get(sprites::Sprite_ID::ORB_SMALL));
    }
