...
namespace views {
    ...
    void SFMLPredictGhost2View::FindSprite() {
        ...
        if (e->getFeared() and e->getFearCheck() < 4.f) {
            ...
            else rect = animation_bool ? sf::IntRect(32, 96, 16, 16) : sf::IntRect(48, 96, 16, 16);
        }
        else if (dirX == 0 && dirY == 1) {
            if (e->getDying()) rect = sf::IntRect(176, 80, 16, 16);
            else rect = animation_bool ? sf::IntRect(96, 96, 16, 16) : sf::IntRect(112, 96, 16, 16);
        }
        else if (dirX == 0 && dirY == -1) {
            if (e->getDying()) rect = sf::IntRect(160, 80, 16, 16);
            else rect = animation_bool ? sf::IntRect(64, 96, 16, 16) : sf::IntRect(80, 96, 16, 16);
        }
        else if (dirX == 1 && dirY == 0) {
            if (e->getDying()) rect = sf::IntRect(128, 80, 16, 16);
            else rect = animation_bool ? sf::IntRect(0, 96, 16, 16) : sf::IntRect(16, 96, 16, 16);
        }
        else {
            if (e->getDying()) rect = sf::IntRect(128, 80, 16, 16);
            else rect = animation_bool ? sf::IntRect(0, 96, 16, 16) : sf::IntRect(16, 96, 16, 16);
