...
void SFMLPredictGhost1View::FindSprite() {
    ...
    if (e->getFeared() and e->getFearCheck() < 4.f) {
        ...
        if (e->getDying())
            ...
            rect = animation_bool ? atlas->get(sprites::Sprite_ID::GHOST_WHITE_LEFT_1)
                                  : atlas->get(sprites::Sprite_ID::GHOST_WHITE_LEFT_2);
    } else if (dirX == 0 && dirY == 1) {
        if (e->getDying())
            rect = atlas->get(sprites::Sprite_ID::GHOST_EYES_DOWN);
        else
            rect = animation_bool ? atlas->get(sprites::Sprite_ID::GHOST_WHITE_DOWN_1)
                                  : atlas->get(sprites::Sprite_ID::GHOST_WHITE_DOWN_2);
    } else if (dirX == 0 && dirY == -1) {
        if (e->getDying())
            rect = atlas->get(sprites::Sprite_ID::GHOST_EYES_UP);
        else
            rect = animation_bool ? atlas->get(sprites::Sprite_ID::GHOST_WHITE_UP_1)
                                  : atlas->get(sprites::Sprite_ID::GHOST_WHITE_UP_2);
    } else if (dirX == 1 && dirY == 0) {
        if (e->getDying())
            rect = atlas->get(sprites::Sprite_ID::GHOST_EYES_RIGHT);
        else
            rect = animation_bool ? atlas->get(sprites::Sprite_ID::GHOST_WHITE_RIGHT_1)
                                  : atlas->get(sprites::Sprite_ID::GHOST_WHITE_RIGHT_2);
    } else {
        if (e->getDying())
            rect = atlas->get(sprites::Sprite_ID::GHOST_EYES_RIGHT);
        else
