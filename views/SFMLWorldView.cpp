...
SFMLWorldView::SFMLWorldView(const sf::Texture &texture,
                             std::shared_ptr<sprites::SpriteAtlas> atlas,
                             sf::RenderWindow &window,
                             std::shared_ptr<Camera> camera)
    : SFMLView(texture, atlas, std::weak_ptr<entities::Entity>(), window,
               camera),
      score(0), lives(3) {
  FindSprite();
...
void SFMLWorldView::Update(float dt) {
  ...
  std::for_each(scoreEntries.begin(), scoreEntries.end(),
                [this, dt](ScoreEntry &entry) {
                  entry.Update(dt);
  ...
  for (const auto &entry : toRemove) {
    scoreEntries.erase(
        std::remove_if(scoreEntries.begin(), scoreEntries.end(),
...
void SFMLWorldView::addScoreEntry(sprites::Sprite_ID ID, float lifetime,
                                  Position position) {
  ScoreEntry entry;
  entry.spriteID = ID;
