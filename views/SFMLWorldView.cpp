...
namespace views {
    ...
    void ScoreEntry::Draw(sf::RenderWindow& window) const {
        window.draw(sprite);
    }
    void ScoreEntry::setScale(float scaleX, float scaleY) {
        sprite.setScale(scaleX, scaleY);
    }
    void ScoreEntry::setPosition(float x, float y) {
        sprite.setPosition(x, y);
    }
    void ScoreEntry::Update(float dt) {
        lifetime -= dt;
    }

    SFMLWorldView::SFMLWorldView(const sf::Texture& texture, std::shared_ptr<sprites::SpriteAtlas> atlas, sf::RenderWindow& window, std::shared_ptr<Camera> camera) : SFMLView(texture, atlas, std::weak_ptr<entities::Entity>(), window, camera), score(0), lives(3) {
        FindSprite();
    ...
    void SFMLWorldView::Update(float dt) {
        ...
        std::for_each(scoreEntries.begin(), scoreEntries.end(), [this, dt](ScoreEntry& entry) {
            entry.Update(dt);
            if (entry.lifetime <= 0.f) { toRemove.push_back(entry); }
        });
        for (const auto& entry : toRemove) {
            scoreEntries.erase(std::remove_if(scoreEntries.begin(), scoreEntries.end(), [&entry](const ScoreEntry& e) { return e == entry; }), scoreEntries.end());}
        toRemove.clear();
    }
