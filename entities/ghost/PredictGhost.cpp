...
float PredictGhost::distanceTurn(std::vector<int> direction, float dt) {
  ...
  float newPacmanX = pacman->getPosition().x +
                     pacman->getDirection()[0] * pacman->getSpeed() * 4 * dt;
  float newPacmanY = pacman->getPosition().y +
                     pacman->getDirection()[1] * pacman->getSpeed() * 4 * dt;
...
void PredictGhost::CalculateNextTurn(float dt) {
  ...
  for (const std::vector<int> &dir_try : IsAtIntersection()) {
    ...
    if ((best_distance == -1.f) ||
        (feared ? dis > best_distance : dis < best_distance)) {
      best_distance = dis;
  ...
  if (best_directions.size() == 1)
    ...
    this->direction =
        best_directions[random.get(0, best_directions.size() - 1)];
  turnTimer = turnSpeed;
}
} // namespace entities