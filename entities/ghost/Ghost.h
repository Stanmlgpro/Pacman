...
public:
  Ghost(float x, float y, std::shared_ptr<Pacman> pacman,
        const std::vector<std::vector<bool>> &wallgrid, int id,
        float chasetime);

  float ManhattanDistance(std::vector<int> direction, Position target,
                          float dt);
  int BFSGridDistance(int startX, int startY, int targetX, int targetY) const;
  float BreathFirstDistance(std::vector<int> direction, Position target,
                            float dt);
