...
namespace entities {
Ghost::Ghost(float x, float y, std::shared_ptr<Pacman> pacman,
...
std::vector<std::vector<int>> Ghost::IsAtIntersection() const {
  ...
  for (const auto &d : dirs) {
    ...
    if (ny < 0 || ny >= static_cast<int>(wallgrid.size()) || nx < 0 ||
        nx >= static_cast<int>(wallgrid[0].size()))
      continue;
  ...
  if (openPaths == 2) {
    ...
    bool right = (gridX < static_cast<int>(wallgrid[0].size()) - 1) &&
                 !wallgrid[gridY][gridX + 1];
    bool up = (gridY > 0) && !wallgrid[gridY - 1][gridX];
    bool down = (gridY < static_cast<int>(wallgrid.size()) - 1) &&
                !wallgrid[gridY + 1][gridX];
...
void Ghost::Update(float dt) {
  if (dying) {
    float EPS = 0.01f;
    if (std::abs(position.x - startpos.x) < EPS &&
        std::abs(position.y - startpos.y) < EPS) {
      setDying(false);
    }
  }
  if (feared) {
...
float Ghost::ManhattanDistance(std::vector<int> direction, Position target,
                               float dt) {
...
int Ghost::BFSGridDistance(int startX, int startY, int targetX,
                           int targetY) const {
...
float Ghost::BreathFirstDistance(std::vector<int> direction, Position target,
                                 float dt) {
