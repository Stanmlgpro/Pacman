...
void MenuState::Render(sf::RenderWindow &window) {
  ...
  title.setPosition(
      window.getSize().x / 2.f - title.getGlobalBounds().width / 2.f, 50.f);
  window.draw(title);

  sf::Text playerText("Name: " + player + "_", font, 30);
  playerText.setFillColor(sf::Color::White);
  playerText.setPosition(window.getSize().x / 2.f -
                             playerText.getGlobalBounds().width / 2.f,
                         150.f);
  window.draw(playerText);

  hsTitle.setPosition(
      window.getSize().x / 2.f - hsTitle.getGlobalBounds().width / 2.f, 220.f);
  window.draw(hsTitle);
  ...
  for (const auto &s : highscores) {
    ...
    entry.setPosition(
        window.getSize().x / 2.f - entry.getGlobalBounds().width / 2.f, y);
    window.draw(entry);
    y += 35.f;
    ++rank;
  }

  hint.setPosition(window.getSize().x / 2.f -
                       hint.getGlobalBounds().width / 2.f,
