...
void World::loadMap_reset() {
  ...
  while (getline(file, line2)) {
    ...
    for (int x = 0; x < static_cast<int>(line2.size()); x++) {
      ...
      if (c == '#') {
        ...
        pacman =
            entity_factory->createPacman(7.5f, width, height, normX, normY);
      } else if (c == 'G') {
  ...
  for (auto &g : ghostSpawns) {
    auto ghost =
        entity_factory->createGhost(g.x, g.y, pacman, wallGrid, g.id, true);
...
std::vector<int>
World::NormalizedToGrid(float normX, float normY,
...
World::CollidesWithPacman(std::shared_ptr<entities::Wall> wall) {
  Position pacPos = pacman->getPosition();
  Position wallPos = wall->getPosition();

  float epsilon = 0.005f;
  float dx =
      std::abs(pacPos.x + pacman->getDirection()[0] * dt - wallPos.x) + epsilon;
  float dy =
      std::abs(pacPos.y + pacman->getDirection()[1] * dt - wallPos.y) + epsilon;

  if (dx < wall->getCollsionSize() / wallGrid[0].size() &&
      dy < wall->getCollsionSize() / wallGrid.size())
    pacman->setMoving(false);
  return nullptr;
}

std::shared_ptr<entities::Entity>
World::CollidesWithPacman(std::shared_ptr<entities::Orb> orb) {
  Position pacPos = pacman->getPosition();
...
std::shared_ptr<entities::Entity>
World::CollidesWithPacman(std::shared_ptr<entities::PowerOrb> powerorb) {
  Position pacPos = pacman->getPosition();
...
std::shared_ptr<entities::Entity>
World::CollidesWithPacman(std::shared_ptr<entities::Fruit> fruit,
...
std::shared_ptr<entities::Entity>
World::CollidesWithPacman(std::shared_ptr<entities::Ghost> ghost) {
  if (ghost->getDying())
    return nullptr;
  Position pacPos = pacman->getPosition();
  ...
  float epsilon = 0.005f;
  float dx = std::abs(pacPos.x + pacman->getDirection()[0] * dt - ghostPos.x) +
             epsilon;
  float dy = std::abs(pacPos.y + pacman->getDirection()[1] * dt - ghostPos.y) +
             epsilon;

  if (dx < ghost->getCollsionSize() / wallGrid[0].size() &&
      dy < ghost->getCollsionSize() / wallGrid.size()) {
    if (ghost->getFeared()) {
      ghost->setDying(true);
      ...
      switch (combo) {
      case 0:
        world_view->ItemEaten(sprites::Sprite_ID::GHOST_EYES_RIGHT,
                              ghost->getPosition());
        break;
      case 1:
        world_view->ItemEaten(sprites::Sprite_ID::GHOST_EYES_LEFT,
                              ghost->getPosition());
        break;
      case 2:
        world_view->ItemEaten(sprites::Sprite_ID::GHOST_EYES_UP,
                              ghost->getPosition());
        break;
      case 3:
        world_view->ItemEaten(sprites::Sprite_ID::GHOST_EYES_DOWN,
                              ghost->getPosition());
        break;
      }
      return nullptr;
...
void World::TryBuffer() {
  ...
  bool canMove = true;
  if (targetGridX < 0 or targetGridX >= static_cast<int>(wallGrid[0].size()) or
      targetGridY < 0 or targetGridY >= static_cast<int>(wallGrid.size()))
    canMove = false;
  ...
  float cellCenterX =
      -1.0f + tileSizeX * (static_cast<float>(currentGridX) + 0.5f);
  float cellCenterY =
      -1.0f + tileSizeY * (static_cast<float>(currentGridY) + 0.5f);
...
bool World::Update() {
  ...
  for (auto &r : removeables) {
    if (r) {
      if (r == pacman)
        gamelost = true;
      entities.erase(std::remove(entities.begin(), entities.end(), r),
                     entities.end());
    }
  }
  for (auto &e : to_add) {
...
std::shared_ptr<entities::Entity>
World::CollidesWithPacman(std::shared_ptr<entities::Pacman> pacman) {
  return nullptr;
}
