...
Game::Game() : window(sf::VideoMode(750, 800), "Pac-Man") {
  ...
  entity_factory = std::make_unique<factory::SFMLFactory>(
      window, "../Spritesheet.png", camera);
  world_sound = std::make_shared<sounds::SFMLWorldSounds>();
  state_manager =
      std::make_unique<states::StateManager>(entity_factory, world_sound);
...
void Game::CheckInput() {
  ...
  while (window.pollEvent(event)) {
    ...
    if (event.type == sf::Event::Resized) {
      ...
      camera->setScreenSize(static_cast<float>(size.x),
